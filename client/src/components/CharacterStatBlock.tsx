...
export type CharacterStatBlockProps = {
    character: any;
};

export const CharacterStatBlock: React.FC<CharacterStatBlockProps> = ({ character }) => {
    const get_senses = () => {
        const passive_perception = 10 + character.skills.find((x: any) => x.name === 'Percepción').modifier;
    ...
    return (
        <div className="CharacterStatBlock">
            ...
            <table>
                <tbody>
                    <tr>
                        {character.ability_scores.map((ability_score: any) => (
            ...
            <div>
                ...
                <span>
                    {character.saving_throws
                        .filter((saving_throw: any) => saving_throw.has_proficiency)
                        .map(
                            (saving_throw: any) =>
            ...
            <div>
                ...
                <span>
                    {character.skills
                        .filter((skill: any) => skill.has_proficiency)
                        .map((skill: any) => `${skill.name} ${skill.modifier >= 0 ? '+' : ''}${skill.modifier}`)
                        .join(', ')}
                </span>
            </div>
            <div>
